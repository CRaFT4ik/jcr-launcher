...
public class ThemeUtils extends BaseUtils {
    ...
    public static void updateStyle(Frame frame)
    {
        ...
        if (!BaseUtils.getPropertyBoolean("offline_mode"))
    ...
    public static JButton doAuth(String name)
    {
        ...
        ActionListener action = new ActionListener() {
            public void actionPerformed(ActionEvent e)
            {
                ...
                } else if (Frame.frame.login.getText().isEmpty() || Frame.frame.login.getText().equals(field_login_title))
                {
                    Frame.frame.setAlert("Введите логин для игры", 3, 0);
                } else
                {
