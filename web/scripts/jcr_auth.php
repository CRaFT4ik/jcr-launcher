<?php
	...
	if ($action == "auth")
	{
		...
		if ($version_compare == (0 | 1))
		{
			...
			/* Forge и LiteLoader теперь будут лежать в папке libraries
			...
			if (!file_exists("../files/clients/$client/bin/minecraft.json"))
				die ("Error: Some elements of client '".$client."' not detected: 'bin/minecraft.json'");
		...
		if ($version_compare == (0 | 1))
		{
			...
			$sha1_md5forge = sha1(md5_file("../files/clients/$client/bin/minecraft.json"));
		...
		} else
		{
			...
			$md5_ass = $sha1_md5forge = $sha1_md5lloader = "null";
			//$md5_lib = "null";
		}
		...
		echo "<br>"."<::>".$programName.$appForm."<::>".$md5_natives."<::>".$md5_ass."<::>"."<br>";
		
		$libs = array(); get_libs_list(null, $libs); if (count($libs) == 0) $libs[] = "nolibs";
		echo implode("<:f:>", $libs)."<::>".$sha1_md5forge."<::>";
		
		echo $sha1_md5lloader."<::>".handle_md5(md5($md5_program))."<::>".$first_hwid_auth."<::>". "<br>" .implode("<:f:>", $configs)."<::>".$getLogin."<br>".
		...
		$db -> query("UPDATE $db_table SET $db_colSesId='$new_ses_id', $db_colAuthId='$seskey', $db_colUUID='$uuid' WHERE $db_colUser='$injLogin'") or die ("Error (database)");
	...
	} else if ($action == "upload_cloak")
	{
		...
		$login = $lowerSkinsCase ? strtolower($getLogin) : $getLogin;
		$move_to_file = "../files/cloaks/cl_".$login.".png";
	...
	function check_access_to_cloak($user_status)
	{
		...
		$login = $lowerSkinsCase ? strtolower($getLogin) : $getLogin;
		if (file_exists("../files/cloaks/cl_".$login.".png"))
			unlink("../files/cloaks/cl_".$login.".png");
		...
		} else if ($type == "cloak")
		{
			...
			if ($cloaks_url != null)
			{
				...
				if (preg_match("|200|", $Headers[0]))
					...
					return $cloaks_url."/cl_default.png";
			}
			
			if (file_exists("../files/cloaks/cl_".$login.".png")) return $login.".png";
			else return "cl_default.png";
		}
	}
	...
	function get_libs_list($folder, $libs)
	{
		global $libs, $client;
		
		$start_folder = "../files/clients/$client/bin/libraries/";
		if (!file_exists($start_folder)) return;
		...
		while ($cv_file = readdir($fp))
		{
			if (is_file($start_folder.$folder."/".$cv_file))
			{
				if (substr($cv_file, -4) == ".jar")
					$libs[] = substr($folder."/".$cv_file, 1). "<:h:>" .md5_file($start_folder.$folder."/".$cv_file);
			} else if ($cv_file != "." && $cv_file != ".." && is_dir($start_folder.$folder."/".$cv_file))
			{
				get_libs_list($folder."/".$cv_file, $libs);
			}
		}
