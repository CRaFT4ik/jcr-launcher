    ...
    private boolean check_for_client_completeness(File dir)
    {
        ...
        for (String object : objects)
        {
            ...
            if (obj.isDirectory() && object.equals("bin"))
            {
                ...
                File libraries = new File(obj + File.separator + "libraries");
                File natives = new File(obj + File.separator + "natives");
                if (minecraft_jar.exists() && libraries.exists() && libraries.list().length != 0 && natives.exists() && natives.list().length != 0)
                {
                    return true; // 1.6.1+
                } else
                {
